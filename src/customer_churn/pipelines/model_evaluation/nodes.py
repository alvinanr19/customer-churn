...
def score(training:pd.DataFrame, testing:pd.DataFrame):
    akurasi = "Akurasi: " + str(accuracy_score(testing.label, testing.prediction)*100) + " %" +"\n"
    presisi = "Presisi: " + str(precision_score(testing.label, testing.prediction)*100) + " %" +"\n"
    recall = "Recall: "+ str(recall_score(testing.label, testing.prediction)*100) + " %" +"\n"
    f1 = "F1-score: " + str(f1_score(testing.label, testing.prediction)*100) + " %" +"\n"
    roc_training = "ROC Training: " + str(roc_auc_score(training['label'], training['probability'])*100) + " %" + "\n"
    roc_testing = "ROC Testing: " + str(roc_auc_score(testing['label'], testing['probability'])*100) + " %"
